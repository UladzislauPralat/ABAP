...
private section.
  ...
  methods CHECK_INVAGINGRANGE
    ...
    raising
      CX_RS_ERROR .
  methods GET_INVAGINGDATE00
    returning
      value(RT_RANGE) type RSR_T_RANGESID .
  methods GET_INVAGINGDATE01
    importing
      !I_VNAM type RSZGLOBV-VNAM
      !I_T_VAR_RANGE type RRS0_T_VAR_RANGE
    returning
      value(RT_RANGE) type RSR_T_RANGESID .
  methods CHECK_INVAGINGDATE00
    importing
      !I_T_VAR_RANGE type RRS0_T_VAR_RANGE
    returning
      value(RT_RANGE) type RSR_T_RANGESID
...
METHOD check.
  ...
  TRY.
    ...
    ENDIF.
    IF line_exists( i_t_var_range[ vnam = 'INVAGINGRANGE1' ] ) OR
       line_exists( i_t_var_range[ vnam = 'INVAGINGRANGE2' ] ) OR
       line_exists( i_t_var_range[ vnam = 'INVAGINGRANGE3' ] ) OR
       line_exists( i_t_var_range[ vnam = 'INVAGINGRANGE4' ] ).
      check_invagingrange( i_t_var_range ).
    ENDIF.
    IF line_exists( i_t_var_range[ vnam = 'INVAGINGDATE00' ] ).
      check_invagingdate00( i_t_var_range ).
    ENDIF.
  CATCH cx_rs_error.
...
METHOD check_invagingdate00.
DATA: w_msgv1 TYPE string.
  TRY.
    DATA(w_date) = CONV d( i_t_var_range[ vnam = 'INVAGINGDATE00' ]-low ).

    IF w_date = sy-datum OR |{ SUBSTRING( val = CONV string( CONV d( w_date + 1 ) ) off = 6 len = 2 ) }| = '01'.
    ELSEIF w_date > sy-datum.
      w_msgv1 = |{ w_date DATE = USER } is in the future|.
      CALL FUNCTION 'RRMS_MESSAGE_HANDLING'
           EXPORTING
                ...
                i_msgv1  = w_msgv1.
    ELSE.
      w_msgv1 = |{ w_date DATE = USER } is not current date or month end|.
      CALL FUNCTION 'RRMS_MESSAGE_HANDLING'
           EXPORTING
                i_class  = 'OO'
                i_type   = 'E'
...
METHOD get_invagingdate01.

  TRY.
    DATA(w_date) = CONV dats( CONV dats( |{ i_t_var_range[ vnam = 'INVAGINGDATE00' ]-low+0(6) }01| ) - 1 ).
    w_date =
      cl_hrpad_date_computations=>add_months_to_date(
        start_date = w_date
        months     = CONV i( i_vnam+12(2) * -1 ) + 1 ).
    rt_range = VALUE #( ( sign = 'I'
                          opt  = 'EQ'
                          low  = |{ w_date }| ) ).
  CATCH cx_sy_itab_line_not_found.
...
METHOD get_invagingrange.
  ...
  rt_range = VALUE #( ( sign = 'I'
                        opt  = 'EQ'
                        low  = SWITCH #( substring( val = i_vnam off = strlen( i_vnam ) - 1 len = 1 )
                                         WHEN '1' THEN '180'
                                         WHEN '2' THEN '360'
                                         WHEN '3' THEN '540'
                                         WHEN '4' THEN '720' ) ) ).
...
METHOD if_rsroa_variables_exit_badi~process.
  ...
  c_t_range =
    COND #( WHEN NOT c_t_range[] IS INITIAL
            THEN c_t_range
            WHEN i_step = 1
            THEN COND #( WHEN i_vnam = 'INVAGINGDATE00'
                         THEN get_invagingdate00( )
            ...
            THEN COND #( WHEN matches( val = i_vnam regex = '^INVAGINGRANGETEXT[1-5]$' )
                         ...
                         WHEN matches( val = i_vnam regex = '^INVAGINGDATE01$' )
                         THEN get_invagingdate01( i_vnam        = i_vnam
                                                  i_t_var_range = i_t_var_range ) )
            WHEN i_step = 3
